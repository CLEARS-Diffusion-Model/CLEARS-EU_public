# CLEARS-EU_public
The CLEARS-EU model estimates national-level adoption pathways for rooftop PV and residential battery storage across 25 EU countries using a two-stage framework that integrates economic evaluation with diffusion dynamics (Hartvig, 2025; Hartvig & Szabó, 2025). In the first stage, the model calculates the net present value (NPV) of investing in PV-connected residential battery systems for heterogeneous households, accounting for region-specific load profiles, consumption patterns, and solar radiation characteristics. In the second stage, potential adopters identified by the NPV analysis are incorporated into Bass diffusion models to simulate cumulative adoption trajectories, capturing non-financial drivers and generating S-shaped growth curves.
